...
# Exclude directories; dynamically include .venv* and always exclude scripts dir to avoid self-delete
$excludeDirs = @('.git','.vscode','__pycache__','venv','env','scripts')
try {
  $venvs = (Get-ChildItem -Force -Directory $Source | Where-Object { $_.Name -like '.venv*' } | Select-Object -ExpandProperty Name)
  if ($venvs) { $excludeDirs += $venvs }
} catch { }
